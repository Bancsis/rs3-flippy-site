...
export async function loadFlips({ bankroll, minVolume = 0, force = false }) {
  ...
  const rows = candidates.map((candidate) => {
    ...
    return {
      ...
      profitPerHour: Number.isFinite(candidate.gpPerHour) && candidate.gpPerHour > 0 ? candidate.gpPerHour : null,
      ...
      details: {
        ...
        expectedFillPerHour: candidate.effectiveRatePerHour,
        fillEstimateSource: candidate.fillEstimateSource,
